...
export function PromiseNotesPanel({
  promiseId,
  initialNotes,
  promiseUpdatedAt,
}: PromiseNotesPanelProps) {
  ...
  const [pendingByNoteId, setPendingByNoteId] = useState<
  ...
  function createNote() {
    ...
    if (!isOnline) {
      void (async () => {
        await queueAction("add_note", {
          promiseId,
          body,
          baseUpdatedAt: promiseUpdatedAt ?? null,
        });
        setNewBody("");
  ...
  function updateNote(noteId: string) {
    ...
    setNotes((prev) =>
      prev.map((note) =>
        note.id === noteId ? { ...note, body: nextBody } : note,
      ),
    );
    ...
    (async () => {
      try {
        const response = await fetch(
          `/api/promises/${promiseId}/notes/${noteId}`,
          {
            method: "PATCH",
            headers: { "content-type": "application/json" },
        ...
        if (!response.ok) {
          ...
          setNotes((prev) =>
            prev.map((note) =>
              note.id === noteId ? { ...note, body: before.body } : note,
            ),
          );
        ...
        setDraft(noteId, before.body);
        setError("You're offline. Note update was not saved.");
      } finally {
        setPendingByNoteId((prev) => {
          const next = { ...prev };
          delete next[noteId];
  ...
  function deleteNote(noteId: string) {
    ...
    (async () => {
      ...
      setError("You're offline. Note deletion was not saved.");
    }finally{
      setPendingByNoteId((prev) => {
        const next = { ...prev };
        delete next[noteId];
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  disabled={
                    Boolean(pendingByNoteId[note.id]) ||
                    note.id.startsWith("tmp-")
                  }
